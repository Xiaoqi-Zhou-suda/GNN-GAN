...
def get_parser():
    ...
    parser.add_argument('--batch_size', type=int, default=10, help='number of batches per epoch')


    parser.add_argument('--imbalance', action='store_true', default=False)
    parser.add_argument('--setting', type=str, default='GAN',
        choices=['no','upsampling', 'smote','recon','GAN'])
    ...
    parser.add_argument('--up_scale', type=float, default=1.5)
...
def split_arti(labels, c_train_num):
    ...
    for i in range(num_classes):
        ...
        train_idx = train_idx + c_idx[:int(0.5*c_train_num[i])]
        c_num_mat[i,0] = c_train_num[i]

        val_idx = val_idx + c_idx[int(0.5*c_train_num[i]):int(0.7*c_train_num[i])]
        test_idx = test_idx + c_idx[int(0.7*c_train_num[i]):c_train_num[i]+80]
...
def split_genuine(labels):
    ...
    for i in range(num_classes):
        ...
        train_idx = train_idx + c_idx[:int(0.25*c_num)]

        val_idx = val_idx + c_idx[int(0.25*c_num):int(0.5*c_num)]
        test_idx = test_idx + c_idx[int(0.5*c_num):c_num]
...
def src_GAN(adj,features,labels,idx_train, im_class_num): #1:203; 5:97
    chosen ={}
    new_features = None
    c_total = np.unique(labels).shape
    c_minor=0
    for k in im_class_num:
        c_minor+=np.sum(labels==k)
    c_major=c_total-c_minor
    ...
    discrete_columns = []
    for i in range(T_train.shape[1] - 1):
        discrete_columns.append(str(i))
    discrete_columns.append('type')
    T_train.columns = discrete_columns
    ctgan = CTGAN(epochs=1000, batch_size=500)
    ctgan.fit(T_train, discrete_columns)
    chosen=None
    new_chosen = ctgan.sample(200)
    new_chosen=np.array(new_chosen)
    adj_back = adj.to_dense()
    for c in range(c_total.shape[0]):
        if c not in im_class_num:
            c_i=np.sum(labels==c)
            c_i_dot=np.sum(new_chosen[:,-1]==c)
            new=int(c_minor*c_i_dot/(c_major-c_i))
            chosen.append(new_chosen[new_chosen==c][:new,:])
        else:
            c_i = np.sum(labels == c)
            c_i_dot = np.sum(new_chosen[:, -1] == c)
            new = int(c_major * c_i_dot / (c_minor - c_i))
            chosen.append(new_chosen[new_chosen == c][:new, :])

    add_num = chosen.shape[0]
    new_adj = adj_back.new(torch.Size((adj_back.shape[0] + add_num, adj_back.shape[0] + add_num))) #创建一个新的Tensor，该Tensor的type和device都和原有Tensor一致，且无内容。
    new_adj[:adj_back.shape[0], :adj_back.shape[0]] = adj_back[:, :]
    features_append = deepcopy(chosen[:, :-1])
    labels_append = deepcopy(chosen[:, -1])
    idx_new = np.arange(adj_back.shape[0], adj_back.shape[0] + add_num)
    idx_train_append = idx_train.new(idx_new)
